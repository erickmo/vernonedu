...
export default function CourseVersionPage() {
  ...
  const rowActions: RowActionDef<Version>[] = [
    {
      ...
      label: 'Edit',
      icon: <Clock size={14} />,
      onClick: (row) => navigate(`/course/${courseId}/versions/${row.id}/edit`),
    },
  ]
  ...
  return (
    <ListPageTemplate<Version>
      ...
      onAdd={() => navigate(`/course/${courseId}/versions/new`)}
      ...
      onRowClick={(row) => navigate(`/course/${courseId}/versions/${row.id}/edit`)}
