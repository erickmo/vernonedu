...
func (h *Handler) Handle(ctx context.Context, cmd commandbus.Command) error {
	c, ok := cmd.(*ApprovePayableCommand)
	if !ok || c == nil {
	...
	if err := h.writeRepo.UpdateStatus(ctx, c.ID, payable.StatusApproved, nil, ""); err != nil {
		return err
	}

	_ = h.bus.Publish(ctx, &payable.PayableApprovedEvent{
