...
func (h *Handler) Handle(ctx context.Context, cmd commandbus.Command) error {
	...
	if err := inv.Cancel(c.Reason); err != nil {
		return err
	}

	if err := h.invoiceWriteRepo.Save(ctx, inv); err != nil {
		log.Error().Err(err).Str("invoice_id", c.InvoiceID.String()).Msg("failed to save cancelled invoice")
		return err
	}

	event := &accounting.InvoiceCancelledEvent{
		InvoiceID: inv.ID,
		Reason:    c.Reason,
		Timestamp: time.Now().UnixMilli(),
	}
	if err := h.eventBus.Publish(ctx, event); err != nil {
		log.Error().Err(err).Msg("failed to publish InvoiceCancelled event")
	}

	log.Info().Str("invoice_id", inv.ID.String()).Msg("invoice cancelled")
	return nil
}
