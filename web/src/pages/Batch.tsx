    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="bg-brand-900 px-12 pt-24 pb-16">
        <div className="max-w-[1200px] mx-auto">
          ...
          <p className="text-[1rem] text-white/45 mt-3 max-w-[480px] leading-relaxed">
      ...
      <div className="border-b border-brand-100 px-12 py-4 bg-white sticky top-16 z-10">
        <div className="max-w-[1200px] mx-auto flex gap-3 flex-wrap items-center">
          <span className="text-xs font-bold text-slate-400 mr-2">Kategori:</span>
          ...
          <span className="text-xs font-bold text-slate-400 ml-4 mr-2">Mode:</span>
          {ALL_MODES.map(m => (
            <button
              key={m}
              onClick={() => setMode(m)}
              className={`text-sm font-bold px-4 py-1.5 rounded-full capitalize transition-colors ${
                mode === m
                  ? 'bg-brand-500 text-white'
                  : 'bg-brand-50 text-brand-500 hover:bg-brand-100'
              }`}
